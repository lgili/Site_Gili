...
def bode_planta_fase(Vi,Ro,L,C,kp,kh,nc2,nc1,nc0,dc2,dc1,dc0,Phase_Mar,Freq_cruz):
  ...
  # Something
  		...
  		x,dado, t = bode(sys_p, Plot=False)
  		conteudo = []
  		dado = dado.tolist()
  		t = t.tolist()
  		for i in range(len(dado)):
...
# 
def step_MF(Vi,Ro,L,C,kp,kh,nc2,nc1,nc0,dc2,dc1,dc0,Phase_Mar,Freq_cruz):
  ...
  # Something
      ...
      #gc = sys_p*sys_c*h
      gc = feedback(sys_p*sys_c, h, sign=-1)

      dado, t = step(gc, Plot=False)
      conteudo = []
      dado = dado.tolist()
      t = t.tolist()
      ...
      dado2, t2 = step(sys_p, T=t, Plot=False)
      conteudo2 = []
      dado2 = dado2.tolist()
      t2 = t2.tolist()

      # dado3, t3 = step(sys_c, T=t, Plot=False)
      # conteudo3 = []
      # dado3 = dado3.tolist()
      # t3 = t3.tolist()
      ...
      for i in range(len(dado2)):
        conteudo2.append({ 'x' : t2[i], 'y': dado2[i]})

      for i in range(len(dado)):
        conteudo3.append({ 'x' : t2[i], 'y': 1-dado[i]})

      return json.dumps([conteudo, conteudo2, conteudo3])



def impulse_MF(Vi,Ro,L,C,kp,kh,nc2,nc1,nc0,dc2,dc1,dc0,Phase_Mar,Freq_cruz):
  ...
  # Something
      ...
      dado2, t2 = impulse(sys_p, T=t, Plot=False)
      conteudo2 = []
      dado2 = dado2.tolist()
      t2 = t2.tolist()
