...
handle_check_full = function() {
    ...
    if (!estado) {
        $("#carga_tag_full").html("Load (&Omega;): " + carga_full.toFixed(2) + " <input id='check_load_full' type='checkbox' onclick='handle_check_full()'>");
    } else {
        $("#carga_tag_full").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_full + "' id='carga_manual_full' onkeydown='if (event.keyCode == 13) play_full()'> <input id='check_load_full' type='checkbox' checked='true' onclick='handle_check_full()'>");
    }
};
...
if (k > full.critico) {
    $("#modo_conducao_full").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_full").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
    k = full.critico;
    saida = saida_full;
    $("#potencia_atual_full").show();
...
///////////////////////////////
    ...
    $("#modo_conducao_full").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_full").html("Capacitance (uF): " + (full.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_full").html("Static Gain: " + (saida / entrada_full_).toFixed(2));
$("#duty_tag_full").html("Duty Cycle: " + full.D.toFixed(2));
$("#indutancia_critica_full").html("Critical Induct. (uH): " + (full.Lcri * 1000000).toFixed(2));
$("#io_param_full").html("k (param.): " + k.toFixed(2));
$("#io_critica_full").html("I<sub>o</sub> critical (A): " + full.Iocrit.toFixed(3));
$("#io_full").html("I<sub>o</sub> (A): " + full.Io.toFixed(2));
$("#indutancia_tag_full").html("Inductance (uH): " + (full.Lo * 1000000).toFixed(2));
handle_check_full();
...
textox('Time (s)','#tensao_saida_full');
textoy('Voltage (V)','#tensao_saida_full');
        ...
        if (previousPoint !== item.datapoint) {
            ...
            if (pos.x > 0 && pos.x < 1 && pos.y > 0 && pos.y < trafo) {
                ...
                if (pos.x > (full.critico)) {
                    $("#valor_ganho_full").html("Static Gain = " + full.q.toFixed(2));
                    $("#continha_full").html("CONTINUOUS");
                    ...
                    $("#valor_novo_carga_full").html("Load (&Omega;) = " + carga_nova);
                    $("#valor_io_novo_full").html("Io (A) = " + ((pos.y * entrada_full_) / carga_nova).toFixed(2));
                } else {
                    $("#valor_ganho_full").html("Static Gain = " + pos.y.toFixed(2));
                    $("#continha_full").html("DISCONTINUOUS");
...
textox('Parametrized Current','#grafico_operacao_full');
textoy('Gain','#grafico_operacao_full');
...
if (cond === "cont") {
    ...
    textox('Time (s)','#corrente_indutor_full');
    textoy('Current (A)','#corrente_indutor_full');
...
//      min: -50
    ...
    textox('Time (s)','#tensao_indutor_full');
    textoy('Voltage (V)','#tensao_indutor_full');
    ...
    var datasetsDiodosA = {
        ...
        function plotAccordingToChoicesDiodosA() {
            ...
            if (data.length > 0) {
                ...
                textox('Time (s)','#corrente_diodo_full');
                textoy('Current (A)','#corrente_diodo_full');
            }
        }
        ...
        var datasetsDiodosV = {
            ...
            function plotAccordingToChoicesDiodosV() {
                ...
                if (data.length > 0) {
                    ...
                    textox('Time (s)','#tensao_diodo_full');
                    textoy('Voltage (V)','#tensao_diodo_full');
                }
            }
            ...
            var datasetsChavesA = {
                ...
                function plotAccordingToChoicesChavesA() {
                    ...
                    if (data.length > 0) {
                        ...
                        textox('Time (s)','#corrente_chave_full');
                        textoy('Current (A)','#corrente_chave_full');
                    }
                }
                ...
                var datasetsChavesV = {
                    ...
                    function plotAccordingToChoicesChavesV() {
                        ...
                        if (data.length > 0) {
                            ...
                            textox('Time (s)','#tensao_chave_full');
                            textoy('Voltage (V)','#tensao_chave_full');
                        }
                    }
...
if (cond == "cont") {
    ...
    textox('Frequence (Hz)','#diagrama_bode_full');
    textoy('Gain (V)','#diagrama_bode_full');

    textox('Frequence (Hz)','#diagrama_fase_full');
    textoy('Angulo (°)','#diagrama_fase_full');
...
textox('Time (s)','#resposta_degrau_full');
textoy('Voltage (V)','#resposta_degrau_full');
