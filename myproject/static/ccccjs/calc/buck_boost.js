...
handle_check_buck_boost = function() {
    ...
    if (!estado) {
        $("#carga_tag_buck_boost").html("Load (&Omega;): " + carga_buck_boost.toFixed(2) + " <input id='check_load_buck_boost' type='checkbox' onclick='handle_check_buck_boost()'>");
    } else {
        $("#carga_tag_buck_boost").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_buck_boost + "' id='carga_manual_buck_boost' onkeydown='if (event.keyCode == 13) play_buck_boost()'> <input id='check_load_buck_boost' type='checkbox' checked='true' onclick='handle_check_buck_boost()'>");
    }
};
...
if (k > buck_boost.critico) {
    $("#modo_conducao_buck_boost").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_buck_boost").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
...
///////////////////////////////
    ...
    $("#modo_conducao_buck_boost").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_buck_boost").html("Capacitance (uF): " + (buck_boost.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_buck_boost").html("Static Gain: " + (saida / entrada_buck_boost).toFixed(2));
$("#duty_tag_buck_boost").html("Duty Cycle: " + buck_boost.D.toFixed(2));
$("#indutancia_critica_buck_boost").html("Critical Induct. (uH): " + (buck_boost.Lcri * 1000000).toFixed(2));
$("#io_param_buck_boost").html("k (param.): " + k.toFixed(2));
$("#io_critica_buck_boost").html("I<sub>o</sub> Critical (A): " + ((buck_boost.critico * entrada_buck_boost * buck_boost.T) / buck_boost.Lo).toFixed(3));
$("#io_buck_boost").html("I<sub>o</sub> (A): " + buck_boost.Io.toFixed(2));
$("#indutancia_tag_buck_boost").html("Indutance (uH): " + (buck_boost.Lo * 1000000).toFixed(2));
...
textox('Time (s)','#tensao_saida_buck_boost');
textoy('Voltage (V)','#tensao_saida_buck_boost');
        ...
        if (previousPoint !== item.datapoint) {
            ...
            if (pos.x > 0 && pos.x < 0.2 && pos.y < 0 && pos.y > -20) {
                ...
                if (pos.x > buck_boost.critico) {
                    $("#valor_ganho_buck_boost").html("Static Gain = " + (saida_buck_boost / entrada_buck_boost).toFixed(2));
                    $("#continha_buck_boost").html("CONTINUOUS");
                    ...
                    $("#valor_novo_carga_buck_boost").html("Load (&Omega;) = " + carga_nova);
                    $("#valor_io_novo_buck_boost").html("Io (A) = " + -((pos.y * entrada_buck_boost) / carga_nova).toFixed(2));
                } else {
                    $("#valor_ganho_buck_boost").html("Static Gain = " + -pos.y.toFixed(2));
                    $("#continha_buck_boost").html("DISCONTINUOUS");
...
textox('Parametrized Current','#grafico_operacao_buck_boost');
textoy('Gain','#grafico_operacao_buck_boost');
...
if (cond === "cont") {
    ...
    textox('Time (s)','#corrente_indutor_buck_boost');
    textoy('Current (A)','#corrente_indutor_buck_boost');
...
//      min: -50
    ...
    textox('Time (s)','#tensao_indutor_buck_boost');
    textoy('Voltage (V)','#tensao_indutor_buck_boost');
    ...
    textox('Time (s)','#corrente_chave_buck_boost');
    textoy('Current (A)','#corrente_chave_buck_boost');
    ...
    textox('Time (s)','#corrente_diodo_buck_boost');
    textoy('Current (A)','#corrente_diodo_buck_boost');
    ...
    textox('Time (s)','#tensao_diodo_buck_boost');
    textoy('Voltage (V)','#tensao_diodo_buck_boost');
    ...
    textox('Time (s)','#tensao_chave_buck_boost');
    textoy('Voltage (V)','#tensao_chave_buck_boost');
...
textox('Frequence (Hz)','#diagrama_bode_buck_boost');
textoy('Gain (V)','#diagrama_bode_buck_boost');

textox('Frequence (Hz)','#diagrama_fase_buck_boost');
textoy('Angule (°)','#diagrama_fase_buck_boost');
...
textox('Time (s)','#resposta_degrau_buck_boost');
textoy('Voltage (V)','#resposta_degrau_buck_boost');
