...
handle_check_boost = function() {
    ...
    if (!estado) {
    $("#carga_tag_boost").html("Load (&Omega;): " + carga_boost.toFixed(2) + " <input id='check_load_boost' type='checkbox' onclick='handle_check_boost()'>");
} else {
    $("#carga_tag_boost").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_boost + "' id='carga_manual_boost' onkeydown='if (event.keyCode == 13) play_boost()'> <input id='check_load_boost' type='checkbox' checked='true' onclick='handle_check_boost()'>");
}
};
...
if(saida_boost<entrada_boost) {
    alert('Output must be greater than input!')
    return false;
}
...
if (k > boost.critico) {
    $("#modo_conducao_boost").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_boost").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
    k = boost.critico;
//  D = boost.D;
...
///////////////////////////////
    ...
    $("#modo_conducao_boost").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_boost").html("Capacitance (uF): " + (boost.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_boost").html("Static Gain: " + (saida / entrada_boost).toFixed(2));
$("#duty_tag_boost").html("Duty Cycle: " + boost.D.toFixed(2));
$("#indutancia_critica_boost").html("Critical Induct. (uH): " + (boost.Lcri * 1000000).toFixed(2));
$("#io_param_boost").html("k (param.): " + k.toFixed(2));
$("#io_critica_boost").html("I<sub>o</sub> Critical (A): " + ((boost.critico * entrada_boost * boost.T) / boost.Lo).toFixed(3));
$("#io_boost").html("I<sub>o</sub> (A): " + boost.Io.toFixed(2));
$("#indutancia_tag_boost").html("Indutance (uH): " + (boost.Lo * 1000000).toFixed(2));
handle_check_boost();
...
textox('Time (s)','#tensao_saida_boost');
textoy('Voltage (V)','#tensao_saida_boost');
...
$("#grafico_operacao_boost").bind("plothover", function(event, pos, item) {
    ...
    if (previousPoint !== item.datapoint) {
        ...
        if (pos.x > boost.critico) {
            $("#valor_ganho_boost").html("Static Gain = " + (saida_boost / entrada_boost).toFixed(2));
            $("#continha_boost").html("CONTINUOUS");
            ...
            $("#valor_novo_carga_boost").html("Load (&Omega;) = " + carga_nova);
            $("#valor_io_novo_boost").html("Io (A) = " + ((pos.y * entrada_boost) / carga_nova).toFixed(2));
        } else {
            $("#valor_ganho_boost").html("Static Gain = " + pos.y.toFixed(2));
            $("#continha_boost").html("DISCONTINUOUS");
...
textox('Parametrized Current ','#grafico_operacao_boost');
textoy('Gain','#grafico_operacao_boost');
...
textox('Time (s)','#corrente_indutor_boost');
textoy('Current (A)','#corrente_indutor_boost');
...
textox('Time (s)','#tensao_indutor_boost');
textoy('Voltage (V)','#tensao_indutor_boost');
...
textox('Time (s)','#corrente_chave_boost');
textoy('Current (A)','#corrente_chave_boost');
...
textox('Time (s)','#corrente_diodo_boost');
textoy('Current (A)','#corrente_diodo_boost');
...
textox('Time (s)','#tensao_diodo_boost');
textoy('Voltage (V)','#tensao_diodo_boost');
...
textox('Time (s)','#tensao_chave_boost');
textoy('Voltage (V)','#tensao_chave_boost');
...
textox('Frequence (Hz)','#diagrama_bode_boost');
textoy('Gain (V)','#diagrama_bode_boost');

textox('Frequence (Hz)','#diagrama_fase_boost');
textoy('Angle (°)','#diagrama_fase_boost');
...
textox('Time (s)','#resposta_degrau_boost');
textoy('Voltage (V)','#resposta_degrau_boost');
