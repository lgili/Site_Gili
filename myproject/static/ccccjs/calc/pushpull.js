...
handle_check_pushpull = function() {
    ...
    if (!estado) {
        $("#carga_tag_pushpull").html("Load (&Omega;): " + carga_pushpull.toFixed(2) + " <input id='check_load_pushpull' type='checkbox' onclick='handle_check_pushpull()'>");
    } else {
        $("#carga_tag_pushpull").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_pushpull + "' id='carga_manual_pushpull' onkeydown='if (event.keyCode == 13) play_pushpull()'> <input id='check_load_pushpull' type='checkbox' checked='true' onclick='handle_check_pushpull()'>");
    }
};
...
if((saida_pushpull / entrada_pushpull)>0.5) {
    alert('Duty Cycle must be less than 0.5!')
    return false;
}
...
if (k > pushpull.critico) {
    $("#modo_conducao_pushpull").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_pushpull").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
...
///////////////////////////////
    ...
    $("#modo_conducao_pushpull").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_pushpull").html("Capacitance (uF): " + (pushpull.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_pushpull").html("Static Gain: " + (saida / entrada_pushpull_).toFixed(2));
$("#duty_tag_pushpull").html("Duty Cycle: " + pushpull.D.toFixed(2));
$("#indutancia_critica_pushpull").html("Critical Induct. (uH): " + (pushpull.Lcri * 1000000).toFixed(2));
$("#io_param_pushpull").html("k (param.): " + k.toFixed(2));
$("#io_critica_pushpull").html("I<sub>o</sub> Critical (A): " + pushpull.Iocrit.toFixed(3));
$("#io_pushpull").html("I<sub>o</sub> (A): " + pushpull.Io.toFixed(2));
$("#indutancia_tag_pushpull").html("Indutance (uH): " + (pushpull.Lo * 1000000).toFixed(2));
...
textox('Time (s)','#tensao_saida_pushpull');
textoy('Voltage (V)','#tensao_saida_pushpull');
        ...
        if (previousPoint !== item.datapoint) {
            ...
            if (pos.x > 0 && pos.x < 1 && pos.y > 0 && pos.y < trafo) {
                ...
                if (pos.x > (pushpull.critico)) {
                    $("#valor_ganho_pushpull").html("Static Gain = " + pushpull.q.toFixed(2));
                    $("#continha_pushpull").html("CONTINUOUS");
                    ...
                    $("#valor_novo_carga_pushpull").html("load (&Omega;) = " + carga_nova);
                    $("#valor_io_novo_pushpull").html("Io (A) = " + ((pos.y * entrada_pushpull) / carga_nova).toFixed(2));
                } else {
                    $("#valor_ganho_pushpull").html("Static Gain = " + pos.y.toFixed(2));
                    $("#continha_pushpull").html("DISCONTINUOUS");
...
textox('Parametrized Current','#grafico_operacao_pushpull');
textoy('Gain','#grafico_operacao_pushpull');
...
if (cond === "cont") {
    ...
    textox('Time (s)','#corrente_indutor_pushpull');
    textoy('Current (A)','#corrente_indutor_pushpull');
...
//      min: -50
    ...
    textox('Time (s)','#tensao_indutor_pushpull');
    textoy('Voltage (V)','#tensao_indutor_pushpull');

    var datasetsDiodosA = {
        "Diodo1Apushpull": {
            color: 6,
            label: "Diode Current 1",
            data: i3_plot
        }, "Diodo2Apushpull":
        {
            color: 12,
            label: "Diode Current  2",
            data: i3_plot_
        }};
        ...
        function plotAccordingToChoicesDiodosA() {
            ...
            if (data.length > 0) {
                ...
                textox('Time (s)','#corrente_diodo_pushpull');
                textoy('Current (A)','#corrente_diodo_pushpull');
            }
        }
        ...
        var datasetsDiodosV = {
            "Diodo1Vpushpull": {
                color: 6,
                label: "Diode Voltage 1",
                data: t1_plot
            }, "Diodo2Vpushpull":
            {
                color: 12,
                label: "Diode Voltage 2",
                data: t1_plot_
            }};
            ...
            function plotAccordingToChoicesDiodosV() {
                ...
                if (data.length > 0) {
                    ...
                    textox('Time (s)','#tensao_diodo_pushpull');
                    textoy('Voltage (V)','#tensao_diodo_pushpull');
                }
            }
            ...
            var datasetsChavesA = {
                "Chave1Apushpull": {
                    color: 6,
                    label: "Switch Current 1",
                    data: i2_plot
                }, "Chave2Apushpull":
                {
                    color: 12,
                    label: "Switch Current 2",
                    data: i2_plot_
                }};
                ...
                function plotAccordingToChoicesChavesA() {
                    ...
                    if (data.length > 0) {
                        ...
                        textox('Time (s)','#corrente_chave_pushpull');
                        textoy('Current (A)','#corrente_chave_pushpull');
                    }
                }
                ...
                var datasetsChavesV = {
                    "Chave1Vpushpull": {
                        color: 6,
                        label: "Switch Voltage 1",
                        data: t2_plot
                    }, "Chave2Vpushpull":
                    {
                        color: 12,
                        label: "Switch Voltage 2",
                        data: t2_plot_
                    }};
                    ...
                    function plotAccordingToChoicesChavesV() {
                        ...
                        if (data.length > 0) {
                            ...
                            textox('Time (s)','#tensao_chave_pushpull');
                            textoy('Voltage (V)','#tensao_chave_pushpull');
                        }
                    }
...
if (cond == "cont") {
    ...
    textox('Frequence (Hz)','#diagrama_bode_pushpull');
    textoy('Gain (V)','#diagrama_bode_pushpull');

    textox('Frequence (Hz)','#diagrama_fase_pushpull');
    textoy('Angulo (°)','#diagrama_fase_pushpull');
...
textox('Time (s)','#resposta_degrau_pushpull');
textoy('Voltage (V)','#resposta_degrau_pushpull');
...
$("#resposta_degrau_pushpull").bind("plothover", function(event, pos, item) {
    ...
    if (item.series.label === "Step Response") {
        if (previousPoint !== item.datapoint) {
            ...
            $("#valor_Time_step_pushpull").html("t (s) = " + pos.x.toFixed(6));
        }
    }
});
}
