...
handle_check_half = function() {
    ...
    if (!estado) {
        $("#carga_tag_half").html("Load (&Omega;): " + carga_half.toFixed(2) + " <input id='check_load_half' type='checkbox' onclick='handle_check_half()'>");
    } else {
        $("#carga_tag_half").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_half + "' id='carga_manual_half' onkeydown='if (event.keyCode == 13) play_half()'> <input id='check_load_half' type='checkbox' checked='true' onclick='handle_check_half()'>");
    }
};
...
if (k > half.critico) {
    $("#modo_conducao_half").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_half").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
    k = half.critico;
    saida = saida_half;
    $("#potencia_atual_half").show();
...
///////////////////////////////
    ...
    $("#modo_conducao_half").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_half").html("Capacitance (uF): " + (half.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_half").html("Static Gain: " + (saida / entrada_half_).toFixed(2));
$("#duty_tag_half").html("Duty Cycle: " + half.D.toFixed(2));
$("#indutancia_critica_half").html("Critical Induct. (uH): " + (half.Lcri * 1000000).toFixed(2));
$("#io_param_half").html("k (param.): " + k.toFixed(2));
$("#io_critica_half").html("I<sub>o</sub> critical (A): " + half.Iocrit.toFixed(3));
$("#io_half").html("I<sub>o</sub> (A): " + half.Io.toFixed(2));
$("#indutancia_tag_half").html("Indutance (uH): " + (half.Lo * 1000000).toFixed(2));
handle_check_half();
...
textox('Time (s)','#tensao_saida_half');
textoy('Voltage (V)','#tensao_saida_half');
        ...
        if (previousPoint !== item.datapoint) {
            ...
            if (pos.x > 0 && pos.x < 1 && pos.y > 0 && pos.y < trafo/2) {
                ...
                if (pos.x > (half.critico)) {
                    $("#valor_ganho_half").html("Static Gain = " + half.q.toFixed(2));
                    $("#continha_half").html("CONTINUOUS");
                    ...
                    $("#valor_novo_carga_half").html("Load (&Omega;) = " + carga_nova);
                    $("#valor_io_novo_half").html("Io (A) = " + ((item.datapoint[1] * entrada_half_) / carga_nova).toFixed(2));
                } else {
                    $("#valor_ganho_half").html("Static Gain = " + pos.y.toFixed(2));
                    $("#continha_half").html("DISCONTINUOUS");
...
textox('Parametrized Current','#grafico_operacao_half');
textoy('Gain','#grafico_operacao_half');
...
if (cond === "cont") {
    ...
    textox('Time (s)','#corrente_indutor_half');
    textoy('Current (A)','#corrente_indutor_half');
...
//      min: -50
    ...
    textox('Time (s)','#tensao_indutor_half');
    textoy('Voltage (V)','#tensao_indutor_half');
    ...
    var datasetsDiodosA = {
        ...
        function plotAccordingToChoicesDiodosA() {
            ...
            if (data.length > 0) {
                ...
                textox('Time (s)','#corrente_diodo_half');
                textoy('Current (A)','#corrente_diodo_half');
            }
        }
        ...
        var datasetsDiodosV = {
            ...
            function plotAccordingToChoicesDiodosV() {
                ...
                if (data.length > 0) {
                    ...
                    textox('Time (s)','#tensao_diodo_half');
                    textoy('Voltage (V)','#tensao_diodo_half');
                }
            }
            ...
            var datasetsChavesA = {
                ...
                function plotAccordingToChoicesChavesA() {
                    ...
                    if (data.length > 0) {
                        ...
                        textox('Time (s)','#corrente_chave_half');
                        textoy('Current (A)','#corrente_chave_half');
                    }
                }
                ...
                var datasetsChavesV = {
                    ...
                    function plotAccordingToChoicesChavesV() {
                        ...
                        if (data.length > 0) {
                            ...
                            textox('Time (s)','#tensao_chave_half');
                            textoy('Voltage (V)','#tensao_chave_half');
                        }
                    }
...
if (cond == "cont") {
    ...
    textox('Frequence (Hz)','#diagrama_bode_half');
    textoy('Gain (V)','#diagrama_bode_half');

    textox('Frequence (Hz)','#diagrama_fase_half');
    textoy('Angulo (°)','#diagrama_fase_half');
...
textox('Time (s)','#resposta_degrau_half');
textoy('Voltage (V)','#resposta_degrau_half');
