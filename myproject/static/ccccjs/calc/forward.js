...
handle_check_forward = function() {
    ...
    if (!estado) {
        $("#carga_tag_forward").html("Load (&Omega;): " + carga_forward.toFixed(2) + " <input id='check_load_forward' type='checkbox' onclick='handle_check_forward()'>");
    } else {
        $("#carga_tag_forward").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_forward + "' id='carga_manual_forward' onkeydown='if (event.keyCode == 13) play_forward()'> <input id='check_load_forward' type='checkbox' checked='true' onclick='handle_check_forward()'>");
    }
};
...
if (k > forward.critico) {
    $("#modo_conducao_forward").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_forward").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
...
///////////////////////////////
    ...
    $("#modo_conducao_forward").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_forward").html("Capacitance (uF): " + (forward.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_forward").html("Static Gain: " + (saida / entrada_forward_).toFixed(2));
$("#duty_tag_forward").html("Duty Cycle: " + forward.D.toFixed(2));
$("#indutancia_critica_forward").html("Critical Induct. (uH): " + (forward.Lcri * 1000000).toFixed(2));
$("#io_param_forward").html("k (param.): " + k.toFixed(2));
$("#io_critica_forward").html("I<sub>o</sub> Critical (A): " + forward.Iocrit.toFixed(3));
$("#io_forward").html("I<sub>o</sub> (A): " + forward.Io.toFixed(2));
$("#indutancia_tag_forward").html("Indutance (uH): " + (forward.Lo * 1000000).toFixed(2));
...
textox('Time (s)','#tensao_saida_forward');
textoy('Voltage (V)','#tensao_saida_forward');
        ...
        if (previousPoint !== item.datapoint) {
            ...
            if (pos.x > 0 && pos.x < 1 && pos.y > 0 && pos.y < trafo) {
                ...
                if (pos.x > forward.critico) {
                    $("#valor_ganho_forward").html("Static Gain = " + forward.q.toFixed(2));
                    $("#continha_forward").html("CONTINUOUS");
                    ...
                    $("#valor_novo_carga_forward").html("Load (&Omega;) = " + carga_nova);
                    $("#valor_io_novo_forward").html("Io (A) = " + ((item.datapoint[1] * entrada_forward_) / carga_nova).toFixed(2));
                } else {
                    $("#valor_ganho_forward").html("Static Gain = " + item.datapoint[1].toFixed(2));
                    $("#continha_forward").html("DISCONTINUOUS");
...
textox('Parametrized Current','#grafico_operacao_forward');
textoy('Gain','#grafico_operacao_forward');
...
if (cond === "cont") {
    ...
    textox('Time (s)','#corrente_indutor_forward');
    textoy('Current (A)','#corrente_indutor_forward');
...
//      min: -50
    ...
    textox('Time (s)','#tensao_indutor_forward');
    textoy('Voltage (V)','#tensao_indutor_forward');
    ...
    function plotAccordingToChoicesDiodosA() {
        ...
        if (data.length > 0) {
            ...
            textox('Time (s)','#corrente_diodo_forward');
            textoy('Current (A)','#corrente_diodo_forward');
        }
    }
    ...
    function plotAccordingToChoicesDiodosV() {
        ...
        if (data.length > 0) {
            ...
            textox('Time (s)','#tensao_diodo_forward');
            textoy('Voltage (V)','#tensao_diodo_forward');
        }
    }
...
if (cond == "cont") {
    ...
    textox('Frequence (Hz)','#diagrama_bode_forward');
    textoy('Gain (V)','#diagrama_bode_forward');

    textox('Frequence (Hz)','#diagrama_fase_forward');
    textoy('Angule (°)','#diagrama_fase_forward');
...
textox('Time (s)','#resposta_degrau_forward');
textoy('Voltage (V)','#resposta_degrau_forward');
...
$("#resposta_degrau_forward").bind("plothover", function(event, pos, item) {
    ...
    if (item.series.label === "Step Response") {
        if (previousPoint !== item.datapoint) {
            ...
            $("#valor_Time_step_forward").html("t (s) = " + pos.x.toFixed(6));
        }
    }
});
}
