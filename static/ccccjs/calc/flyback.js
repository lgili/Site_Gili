...
handle_check_flyback = function() {
    ...
    if (!estado) {
        $("#carga_tag_flyback").html("Load (&Omega;): " + carga_flyback.toFixed(2) + " <input id='check_load_flyback' type='checkbox' onclick='handle_check_flyback()'>");
    } else {
        $("#carga_tag_flyback").html("Load (&Omega;): <input type='text' style='width: 50px;' value='" + carga_manual_flyback + "' id='carga_manual_flyback' onkeydown='if (event.keyCode == 13) play_flyback()'> <input id='check_load_flyback' type='checkbox' checked='true' onclick='handle_check_flyback()'>");
    }
};
...
///////////////////////////////
// MODO DE CONDUÃ‡ÃƒO
// neste momento sÃ£o efetuados prÃ©-calculos pro plot operaÃ§Ã£o
///////////////////////////////
...
if (k > flyback.critico) {
    $("#modo_conducao_flyback").html("<pre>CONTINUOUS CONDUCTION MODE*</pre>");
...
///////////////////////////////
    ...
    $("#modo_conducao_flyback").html("<pre>CRITICAL CONDUCTION MODE*</pre>");
    cond = "crit";
    k = critico;
    saida = saida_flyback;
$("#potencia_atual_flyback").show();
...
///////////////////////////////
    ...
    $("#modo_conducao_flyback").html("<pre>DISCONTINUOUS CONDUCTION MODE*</pre>");
...
$("#capacitancia_tag_flyback").html("Capacitance (uF): " + (flyback.capacitancia * 1000000).toFixed(2));
$("#ganho_tag_flyback").html("Static Gain: " + (saida / entrada_flyback_).toFixed(2));
$("#duty_tag_flyback").html("Duty Cycle: " + flyback.D.toFixed(2));
$("#indutancia_critica_flyback").html("Critical Induct. (uH): " + (flyback.Lcri * 1000000).toFixed(2));
$("#io_param_flyback").html("k (param.): " + k.toFixed(2));
//$("#io_critica_flyback").html("I<sub>o</sub> crÃ­tica (A): " + ((flyback.critico * entrada_flyback_ * flyback.T) / flyback.Lo).toFixed(3));
$("#io_critica_flyback").html("I<sub>o</sub> Critical (A): " + flyback.Iocrit.toFixed(3));
$("#io_flyback").html("I<sub>o</sub> (A): " + flyback.Io.toFixed(2));
$("#indutancia_tag_flyback").html("Inductance (uH): " + (flyback.Lo * 1000000).toFixed(2));
handle_check_flyback();

///////////////////////////////
// PLOT DA TENSÃƒO NA CARGA
///////////////////////////////
...
textox('Time (s)','#tensao_saida_flyback');
textoy('Voltage (V)','#tensao_saida_flyback');
        ...
        if (previousPoint !== item.datapoint) {
            ...
            if (pos.x > 0 && pos.x < 0.2 && pos.y > 0 && pos.y < 20) {
                ...
                if (pos.x > flyback.critico) {
                    $("#valor_ganho_flyback").html("Static Gain = " + (saida_flyback / entrada_flyback).toFixed(2));
                    $("#continha_flyback").html("CONTINUOUS");
                    ...
                    $("#valor_novo_carga_flyback").html("Load (&Omega;) = " + carga_nova);
                    $("#valor_io_novo_flyback").html("Io (A) = " + ((pos.y * entrada_flyback_) / carga_nova).toFixed(2));
                } else {
                    $("#valor_ganho_flyback").html("Static Gain = " + pos.y.toFixed(2));
                    $("#continha_flyback").html("DISCONTINUOUS");
...
//if (item && (item.series.label === "Curva OperaÃ§Ã£o")) {
...
textox('Parametrized Current','#grafico_operacao_flyback');
textoy('Gain','#grafico_operacao_flyback');

///////////////////////////////
// PLOT DAS CORRENTES E TENSÃ•ES
///////////////////////////////
...
if (cond === "cont") {
    ...
    textox('Time (s)','#corrente_indutor_flyback');
    textoy('Current (A)','#corrente_indutor_flyback');
...
//      min: -50
    ...
    textox('Time (s)','#tensao_indutor_flyback');
    textoy('Voltage (V)','#tensao_indutor_flyback');
    ...
    textox('Time (s)','#corrente_chave_flyback');
    textoy('Current (A)','#corrente_chave_flyback');
    ...
    textox('Time (s)','#corrente_diodo_flyback');
    textoy('Current (A)','#corrente_diodo_flyback');
    ...
    textox('Time (s)','#tensao_diodo_flyback');
    textoy('Voltage (V)','#tensao_diodo_flyback');
    ...
    textox('Time (s)','#tensao_chave_flyback');
    textoy('Voltage (V)','#tensao_chave_flyback');
...
textox('Frequence (Hz)','#diagrama_bode_flyback');
textoy('Gain (V)','#diagrama_bode_flyback');

textox('Frequence (Hz)','#diagrama_fase_flyback');
textoy('Angulo (°)','#diagrama_fase_flyback');
...
textox('Time (s)','#resposta_degrau_flyback');
textoy('Voltage (V)','#resposta_degrau_flyback');
